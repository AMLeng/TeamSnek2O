...
def distorted_inputs():
    ...
    """
    images, labels = steer_input.distorted_inputs(data_dir=DATA_DIR,batch_size=BATCH_SIZE)
...
def inference(images):
    ...
    with tf.variable_scope('conv2') as scope:
        kernel = _variable_with_weight_decay('weights',shape=[5, 5, 24, 36],stddev=5e-2,wd=0.0)
    ...
    with tf.variable_scope('conv3') as scope:
        kernel = _variable_with_weight_decay('weights',shape=[5, 5, 36, 48],stddev=5e-2,wd=0.0)
    ...
    with tf.variable_scope('conv4') as scope:
        kernel = _variable_with_weight_decay('weights',shape=[3, 3, 48, 64],stddev=5e-2,wd=0.0)
    ...
    with tf.variable_scope('local1') as scope:
        ...
        weights = _variable_with_weight_decay('weights', shape=[dim, 100],stddev=0.04, wd=0.0)
        biases = _variable_on_cpu('biases', [100], tf.constant_initializer(0.1))
        local1 = tf.nn.relu(tf.matmul(reshape, weights) + biases, name=scope.name)

    # local2
    with tf.variable_scope('local2') as scope:
        weights = _variable_with_weight_decay('weights', shape=[100, 50],stddev=0.04, wd=0.0)
    ...
    with tf.variable_scope('local3') as scope:
        weights = _variable_with_weight_decay('weights', shape=[50, 10],stddev=0.04, wd=0.0)
